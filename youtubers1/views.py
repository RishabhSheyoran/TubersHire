...
def youtubers1_detail(request,id):
...
def search(request):
    ...
    category_search = Youtuber.objects.values_list('cateogry',flat=True).distinct()
    ...
    if 'cateogry' in request.GET:
        cateogry= request.GET['cateogry']
        if cateogry:
             tubers = tubers.filter(cateogry__iexact= city)
